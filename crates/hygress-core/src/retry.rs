...
pub enum RetryCond {
    ...
    Status(u16),
    /// Allow retrying non-idempotent requests (gated on the caller flag in
...
impl RetryPolicy {
    ...
    pub fn parse(conditions: Option<&str>, tries: Option<&str>) -> ParsedRetry {
        ...
        let tries = match tries {
            None => 2,
            Some(raw) => raw.trim().parse::<u32>().unwrap_or(2),
        };
    ...
    /// - `non_idempotent` — the HTTP method is non-idempotent (e.g. POST
    ///   without `Idempotency-Key`); only relevant when the policy contains
    ///   [`RetryCond::NonIdempotent`].
    pub fn should_retry(
        &self,
        status: Option<u16>,
        ...
        for c in &self.conditions {
            match c {
                ...
                RetryCond::Http5xx => {
                    if status.is_some_and(|s| (500..=599).contains(&s)) {
                        return true;
                    }
                }
                RetryCond::Status(code) => {
                    if status == Some(*code) {
                        return true;
                    }
                }
...
mod tests {
    ...
    fn should_retry_default_matrix() {
        ...
        // transport error / timeout / non_idempotent
        assert!(default.should_retry(None, true, false, false));
        assert!(default.should_retry(None, false, true, false));
        // POST is non-idempotent; default policy permits it via NonIdempotent.
