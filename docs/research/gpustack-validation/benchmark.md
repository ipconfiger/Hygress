...
**修复方向（未实施，记档）**：将 `RouteTable` 按 `ConfigData` 修订号缓存——表仅在快照变更（秒级热加载/store）
时重建一次，请求路径只做一次 Arc 读取（`pipe.rs:272` 改为取缓存）。预计可消除该镜像路径的大部分差距；
是否实施由后续决策决定，与此前的诚实结论（"同量级、不判定胜负"）一致。
