...
> 2. 网关内核下限 13.6k req/s、p99 6ms 平坦（`§11` 自服务 healthz 判定器）。`:80/readyz` 的 ~1.1k req/s 与周期性 p99 尾部归因于共享镜像上游而非网关内核，两侧在镜像路径上呈现同款尾部。
...
- 两侧在 `:80/readyz` 上都有周期性 ~300-500ms p99 尾，归因判定见 §1.2（非网关内核所致）。
...
判定：网关内核（pingora accept+parse+respond，不经过 request_filter 管线/路由/上游）平坦且快 ~12×，p99 个位数毫秒、无周期尾。`:80/readyz` 的吞吐天花板（~1.1k）与周期性尾部**归因于共享镜像上游**（GPUStack worker 的 `/readyz` 往返与抖动）；envoy 侧同路径同样呈现尾部（407/530ms），两侧同等“taily”。§8→§10 期间逐项消除 P1（连接churn）、P2（单线程）、P4（控制面轮询）等网关侧开销后，`:80/readyz` 的 p99 只随上游波动。
...
P1 根治 read-errors（close-delimited framing 根因，pingora `init_close_delimited` 实证）；P2 消除单 worker 串行化（c64 +7.7%）；P4 消除控制面周期重建尖峰（p99 −23/−25%）；WATCH 后尾未扁平、§11 内核平坦，周期尾最终归因共享上游（非网关）；同 rig 下 hygress 全指标 ≥ envoy。
