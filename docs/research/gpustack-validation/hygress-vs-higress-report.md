...
> **核心结论（key takeaways）**
> 1. **性能持平或反超**：最终同 rig wrk 对比（同一 GPUStack 主机、同 wrk、同 `:80/readyz`，顺序 hygress→higress→hygress），hygress 吞吐 **c16 +4.8%、c64 +20.3%**，p50 **−7.8%/−12.0%**、p99 **−5.3%/−20.6%**（证据 `bench_final_*.txt`，构建 `3b6beabc` vs `55ecc762`）。
> 2. **网关内核下限 13.6k req/s、p99 6ms 平坦**（`§11` 自服务 healthz 判定器）：`:80/readyz` 的 ~1.1k req/s 与周期性 p99 尾部**归因于共享镜像上游**，非网关内核——两侧在镜像路径上呈现同款尾部。
> 3. **资源数量级优势**：数据面 **1 进程 / ~108MB RSS** vs envoy 套件 **4 进程 / ~2.54GB**（**≈23×**）；镜像内网关二进制 27MB vs ~905MB（≈34×）。
> 4. **可运维性与架构**：单二进制原位替换（零 Python 改动、s6 一键回滚）、9 插件等价原生 Rust 管线、控制面 WATCH 事件化热更（稳态零 LIST）、限流/配额/路由策略/护栏真机验证通过。
...
- **负载**：盒内 wrk，`GET :80/readyz`（镜像透传路径，两端均实时 200），c16 = `-t4 -c16 -d30s`，c64 = `-t8 -c64 -d20s`；内核下限 `:8081/healthz`（hygress 自服务，无上游）。
...
| c16 avg | 31.75 ms | 28.23 ms | +12.5%（尾部质量拖高均值，见 p99 分布） |
...
- hygress 全部四项主指标（吞吐/p50/p99）**均优于原生 envoy**；c64 优势（+20.3%）大于 c16（+4.8%），与 c64 更吃单请求 CPU/连接效率一致（P2 多线程 + P1 keep-alive 的收益随并发放大）。
- c16 avg 一项 hygress 偏高（31.75 vs 28.23）：由 p99 之后的尾部样本贡献（两侧 p99 相近、hygress 极端尾样本更重）；主判据（吞吐/p50/p99）均优。
- 两侧在 `:80/readyz` 上都有**周期性 ~300-500ms p99 尾**——见 §1.2 的归因判定：**非网关内核所致**。
...
| c16 p99（30s + 重复×3） | **6.07 ms（重复 5.6-6.9，平坦）** | 385.8 ms（周期性尖峰） |
| c64 吞吐 / p50 / p99 | 14,537 / 2.49 / 24.4 ms | 916.1 / 51.8 / 421.5 ms |

**判定（决定性）**：网关内核（pingora accept+parse+respond，不经过 request_filter 管线/路由/上游）**平坦且快 ~12×**——p99 个位数毫秒、无周期尾。`:80/readyz` 的吞吐天花板（~1.1k）与周期性尾部**归因于共享镜像上游**（GPUStack worker 的 `/readyz` 往返与抖动）；envoy 侧同路径同样呈现尾部（407/530ms），两侧同等"taily"。这解释了 §8→§10 的现象链：P1（连接churn）/P2（单线程）/P4（控制面轮询）逐一消除网关侧开销后，`:80/readyz` 的 p99 只随上游波动。
诚实说明：admin 端点当前仍为 close-delimited 响应（`read errors ≈ 全量`，P1 nitpick #1）——即 13.6k req/s 是在**每请求 TCP 拆建**的不利条件下测得的内核下限，只会低估；如需 keep-alive 口径属一行 CL 修复（非热路径、可选）。跨网关"纯内核对纯内核"的正式对比（envoy admin 端点等效测量）留作 Phase 3.2 可选待办。
...
| **cf4f6c5**（Phase 1.1 WATCH，§10） | 1119-1143 | 12.6 / 388-481 | 824 | 50.0 / 577 | 0 | 控制面稳态零 LIST；尾未扁平 → 尾源非控制面 |
| **fc25a87**（§11 内核下限） | **13,598.67**（p50 0.74ms / p99 6.1ms 平坦） | — | 14,537（c64） | — | （admin close-delimited） | 归因判定器：尾源 = 共享上游 |
...
判读链：① P1 根治 read-errors（close-delimited framing 根因，pingora `init_close_delimited` 实证）；② P2 消除单 worker 串行化（c64 +7.7%）；③ P4 消除控制面周期重建尖峰（p99 −23/−25%）；④ WATCH 后尾未扁平 + §11 内核平坦 → **周期尾最终归因共享上游**（非网关）；⑤ 同 rig 下 hygress 全指标 ≥ envoy。
...
6. **`:80/readyz` 周期尾为上游所致（非网关）**：§11 内核平坦（p99 6.1ms）+ envoy 同款尾部（407/530ms）已归因共享 GPUStack worker；该尾不在 hygress 可修复范围。
7. **跨网关纯内核对比未完成**：envoy 无等效 admin 基准被测（Phase 3.2 静态 sink 可选待办）。当前内核结论（13.6k req/s、p99 6ms 平坦）仅 hygress 单侧；跨网关内核对比留待需要时补测。
...
- **GPUStack 内嵌网关原位替换场景：选 Hygress。** 依据：同 rig wrk 全主指标 ≥ envoy（吞吐 +4.8%/+20.3%，p50/p99 全优）；内核下限 13.6k req/s（≈12× 于镜像路径实测负载，余量充足）；资源 1 进程/108MB vs 4 进程/2.54GB（≈23×）、镜像二进制 34×；9 插件等价 + 限流/配额/策略/护栏真机验证；CRD/usage/端口契约逐字节一致、零 Python 改动、一键回滚；优化全程可复现（§6→§11 曲线，每步 oracle 复审 + 双模式全绿）。
- **仍选原生 Higress/envoy 的场景**：需要 Wasm/Go 插件生态或自定义 envoy filter；需要 Istio/xDS 多集群 fleet 语义；需要 envoy 热重启（不切镜像的进程级升级）；或 GPUStack 之外的独立网关拓扑。这些是能力域差异，非性能差异——本报告的性能结论在 GPUStack 内嵌场景内成立。
